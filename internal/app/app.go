...
func Run() {
	...
	pageHandler := handler.NewPageHandler(postService, boardService).WithComments(commentService).WithClubs(clubService).WithUsers(userRepo)
	...
	r.HandleFunc("/logout", pageHandler.LogoutHandler).Methods(http.MethodGet)
